...
enum RunState
{
  ...
  arm_go_home,             // 机械臂回参考点
  wait_for_car_to_go_to_point, // QRCode_P1，storage_area_P2，roughing_area_red_P3,roughing_area_green_P4,roughing_area_blue_P5,semifinished_area_red_P7
  ...
  if (isSlave78Online)
  {
    ...
    case arm_go_home:
      ...
      if (true)
      {
        ...
        run_state = wait_for_car_to_go_to_point;
      }

      break;

    case wait_for_car_to_go_to_point:
      
      if (get_wmr_status() == 0x66)//等待到位后再前往下一点
      {
        switch (pointNUM)
        {
        case 0:
    ...
    case tell_CAM_to_get_object_color:
      ...
      */
      if (true)
      {
        run_state = pick_place_Object_from_storage_area;
      }

      break;

    case pick_place_Object_from_storage_area:

      // pick_place_Object_from_storage_area
