...
#define FSUS_CMD_WRITE_BATCH_DATA \
  6  // 单个舵机 批次写入(写入一个舵机所有的数据)
...
#define FSUS_STATUS_SIZE_TOO_BIG \
  5  // 参数的size大于FSUS_PACK_RESPONSE_MAX_SIZE里面的限制
...
// 舵机限制原始角度绝对值 sunnyadd
#define FSUS_PARAM_RAW_ANGLE_LIMIT 135
/* 用户只读区域 */
...
/**
 * 舵机型号 数据类型：uint16_t。SUNNY ADD
 */
#define FSUS_PARAM_TYPE 6
/**
 * 舵机固件版本 数据类型：uint16_t。SUNNY ADD
 */
#define FSUS_PARAM_FIRMWARE 7
/**
 * 舵机序列号(serial_number)舵机的唯一识别符 数据类型：uint32_t。SUNNY ADD
 */
#define FSUS_PARAM_SN 8
...
class FSUS_Protocol {
 public:
  ...
  // 发送旋转的请求包
  void sendSetAngle(FSUS_SERVO_ID_T servoId, FSUS_SERVO_ANGLE_T angle,
                    FSUS_INTERVAL_T interval, FSUS_POWER_T power);
  // 发送旋转的请求包(指定周期)
  void sendSetAngleByInterval(FSUS_SERVO_ID_T servoId, FSUS_SERVO_ANGLE_T angle,
                              FSUS_INTERVAL_T interval, FSUS_INTERVAL_T t_acc,
                              FSUS_INTERVAL_T t_dec, FSUS_POWER_T power);
  // 发送旋转的请求包(指定转速)
  void sendSetAngleByVelocity(FSUS_SERVO_ID_T servoId, FSUS_SERVO_ANGLE_T angle,
                              FSUS_SERVO_SPEED_T velocity,
                              FSUS_INTERVAL_T t_acc, FSUS_INTERVAL_T t_dec,
                              FSUS_POWER_T power);
  // 发送舵机角度查询指令
  void sendQueryAngle(FSUS_SERVO_ID_T servoId);
  // 接收角度查询指令
  FSUS_STATUS recvQueryAngle(FSUS_SERVO_ID_T *servoId,
                             FSUS_SERVO_ANGLE_T *angle);
  // 发送旋转的请求包(多圈)
  void sendSetAngleMTurn(FSUS_SERVO_ID_T servoId, FSUS_SERVO_ANGLE_T angle,
                         FSUS_INTERVAL_T_MTURN interval, FSUS_POWER_T power);
  // 发送旋转的请求包(多圈+指定周期)
  void sendSetAngleMTurnByInterval(FSUS_SERVO_ID_T servoId,
                                   FSUS_SERVO_ANGLE_T angle,
                                   FSUS_INTERVAL_T_MTURN interval,
                                   FSUS_INTERVAL_T t_acc, FSUS_INTERVAL_T t_dec,
                                   FSUS_POWER_T power);
  // 发送旋转的请求包(多圈+指定转速)
  void sendSetAngleMTurnByVelocity(FSUS_SERVO_ID_T servoId,
                                   FSUS_SERVO_ANGLE_T angle,
                                   FSUS_SERVO_SPEED_T velocity,
                                   FSUS_INTERVAL_T t_acc, FSUS_INTERVAL_T t_dec,
                                   FSUS_POWER_T power);
  ...
  // 接收角度查询指令(多圈模式)
  FSUS_STATUS recvQueryAngleMTurn(FSUS_SERVO_ID_T *servoId,
                                  FSUS_SERVO_ANGLE_T *angle);
  // 发送阻尼模式
  void sendDammping(FSUS_SERVO_ID_T servoId, FSUS_POWER_T power);
  // 轮式控制模式
  void sendWheelMove(FSUS_SERVO_ID_T servoId, uint8_t method, uint16_t speed,
                     uint16_t value);
  // 轮子停止转动
  void sendWheelStop(FSUS_SERVO_ID_T servoId);
  // 轮子持续旋转
  void sendWheelKeepMove(FSUS_SERVO_ID_T servoId, uint8_t is_cw,
                         uint16_t speed);
  // 指定旋转的时间
  void sendWheelMoveTime(FSUS_SERVO_ID_T servoId, uint8_t is_cw, uint16_t speed,
                         uint16_t nTime);
  // 轮式模式 旋转特定的圈数
  void sendWheelMoveNCircle(FSUS_SERVO_ID_T servoId, uint8_t is_cw,
  ...
  // 接收数据读取指令
  FSUS_STATUS recvReadData(FSUS_SERVO_ID_T *servoId, uint8_t *address,
                           uint8_t *contentLen, uint8_t *content);
  // 发送数据写入指令
  void sendWriteData(FSUS_SERVO_ID_T servoId, uint8_t address,
                     uint8_t contentLen, uint8_t *content);
  // 接收数据写入指令
  FSUS_STATUS recvWriteData(FSUS_SERVO_ID_T *servoId, uint8_t *address,
                            bool *result);
};
#endif
