...
#define FSUS_WAIT_TIMEOUT_MS 1000     // 等待的时间上限 ms
...
class FSUS_Servo {
 public:
  ...
  //原始角度限制 sunny add 通过查询获得
  FSUS_SERVO_ANGLE_T raw_angleMin;  //舵机原始角度的最小值
  FSUS_SERVO_ANGLE_T raw_angleMax;  // 舵机原始角度的最大值
  ...
  bool ping();
  // 舵机标定
  void calibration(FSUS_SERVO_ANGLE_T rawA, FSUS_SERVO_ANGLE_T realA,
  ...
  // 限制舵机期望原始角度范围到舵机原始角度方位内 在范围内返回1 范围外 限制到范围内后 返回0  sunny add
  bool constrainRawAngle(FSUS_SERVO_ANGLE_T &rawAngle);
  // 设置舵机的角度范围
  ...
  // 设置舵机的原始角度(指定周期)
  void setRawAngleByInterval(FSUS_SERVO_ANGLE_T rawAngle,
                             FSUS_INTERVAL_T interval, FSUS_INTERVAL_T t_acc,
                             FSUS_INTERVAL_T t_dec, FSUS_POWER_T power);
  // 设定舵机的原始角度(指定转速)
  void setRawAngleByVelocity(FSUS_SERVO_ANGLE_T rawAngle,
  ...
  void setRawAngleMTurn(FSUS_SERVO_ANGLE_T rawAngle);
  // 设定舵机的原始角度(多圈+指定周期)
  void setRawAngleMTurnByInterval(FSUS_SERVO_ANGLE_T rawAngle,
                                  FSUS_INTERVAL_T_MTURN interval,
                                  FSUS_INTERVAL_T t_acc, FSUS_INTERVAL_T t_dec,
                                  FSUS_POWER_T power);
  // 设定舵机的原始角度(多圈+指定转速)
  void setRawAngleMTurnByVelocity(FSUS_SERVO_ANGLE_T rawAngle,
  ...
  // 舵机型号 2字节 sunny add
  uint32_t queryType();
  // 查询舵机固件版本 2字节  sunny add
  uint32_t queryFirmware();
  // 查询舵机的SN 4字节 sunny add
  uint32_t querySN();
  // 设置阻尼模式
