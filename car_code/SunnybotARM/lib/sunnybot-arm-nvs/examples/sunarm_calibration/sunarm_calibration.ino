...
//程序功能：一键标定
//使用方法：1.上传data下网页文件 2.上传程序 3.手动调整机械臂到竖直态，然后按下标定按钮IO0,IO0状态:0;累计次数:11
//4.标定后手动扳动机械臂各关节，查看各关节角度变化方向是否和零构型一致，不一致则修改方向0 1,166行bool flipTurn[] = {0, 0, 1, 1};
//设置手爪角度和连杆长度
//测试网页连接
//设计一个竖直标定设备
//经常有个别通讯不上的问题
...
// to do 拖动示教保存成文件
//to do 添加手爪校准？
//注意：若舵机无响应，全部断电后再测试，包括usb供电
//前提：网页上传完成
//wifi连接后，浏览器打开：192.168.4.1查看角度变化
...
const char *ssid = "ESP32-SUNARM-0719";
...
void setup() {
  ...
  pinMode(ButtonPin, INPUT_PULLUP);//按钮引脚模式设置
  initWiFi();
  initSPIFFS();
  arm.init();            //机械臂初始化
  arm.gripperOpen();   //测试手爪开启，可带参数
  ...
  //标定数据   servo 0  1  2  3
  bool flipTurn[] = {0, 0, 1, 1};//根据手掰角度与建模角度是否一致，调整舵机极性。
