...
// 串口总线舵机配置参数
#define RT_SERVO_ID 5 //舵机ID号
#define RT_BAUDRATE 115200 // 波特率
FSUS_Protocol rtprotocol(RT_BAUDRATE); //协议
FSUS_Servo rtservo(RT_SERVO_ID, &rtprotocol); // 创建舵机
...
void setup()
{
  ...
  arm.gripperOpen(); //测试手爪开启
    ...
    rtprotocol.init(); // 通信协议初始化
    rtservo.init(); //舵机角度初始化
...
//执行动作组0
  ...
  for (size_t i = 0; i < actionsLenth; i++)
  {
    ...
    {
      arm.gripperOpen();
      rtservo.setRawAngle(90.0);  // 设置舵机的角度
    }
    else
    {
      arm.gripperClose();
