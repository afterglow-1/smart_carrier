...
void setup() {
  ...
  // jy61陀螺仪采集串口1,默认9600
  Serial1.begin(9600, SERIAL_8N1, RXD1, TXD1);  //二合一版
  /*
  //尝试修改为115200，失败？
  Serial1.write(unlock, 5);                    //解锁
  Serial1.flush();//等待发送完成
  Serial1.write(setBaud, 5);                    //设置波特率为115200
  Serial1.flush();//等待发送完成
  Serial1.write(saveData, 5);                    //保存
  Serial1.flush();//等待发送完成
  //delay(10);
  //按115200设置
 Serial1.begin(115200, SERIAL_8N1, RXD1, TXD1);  //二合一版
 //Serial1.updateBaudRate(115200);//重新设置波特率

 */
  Serial1.write(Horizontal, 3);                   // Z轴角度复位指令
Serial1.flush();//等待发送完成
  Serial1.write(resetZAngle, 3);  // Z轴角度复位指令
  Serial1.flush();//等待发送完成
...
void getPIDAngularVelocity() {
  ...
  angular_vel_z = -realYaw * 0.1;//系数根据角度刷新率修改
}
//通过逆运动学得到电机转速（脉冲数）
void getMotorSpeed() {
