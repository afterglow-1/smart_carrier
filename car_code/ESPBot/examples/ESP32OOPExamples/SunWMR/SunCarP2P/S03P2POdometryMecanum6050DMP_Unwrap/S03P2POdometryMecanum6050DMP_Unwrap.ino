...
//走三个点180度测试 使用枚举类型 平移相位角度 可连续多圈旋转
#include <Arduino.h>
...
#include "MPU6050_6Axis_MotionApps612.h" //和I2Cdev.h同时安装了,原版I2Cdev无法使用
MPU6050 mpu;
#endif
...
CAR_GOAL_POINT p2{900, 900, M_PI};
//比例系数和速度最大值
...
if (fabs(newYawRad-lastYaw)>M_PI)//如果差值大于M_PI，平移相位 每当连续相位角之间的跳跃大于π 弧度时，通过增加 ±2π 的整数倍来平移相位角，确保跳动小于 π。
{
 ...
 {   
  ...
  if (isFirst)
  {
    initialYawRad = newYawRad; //初始角度
    isFirst = 0;
  }
  realYawRad = newYawRad - initialYawRad; // realYawRad 和初始角度的偏离角度， 传入里程计（定时器中）
