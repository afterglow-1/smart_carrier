...
void setup() {
  ...
  //                        X    Y    Z
  botOdometry.calibrationK(1.0048, 1.0, 1.0);
  botOdometry.setPositon_mm(100,100,0);
...
void loop() {
  ...
  if (timer_flag) {
    ...
    updateSensors();         // 更新JY61P传感数据
    //getFlowXYVelocity();      // 通过光流数据，使用PID控制器得到XY校准速度，仅使用比例环
    getIMUAngularVelocity();  // 通过PID控制器得到角速度，仅使用比例环
...
void updateTargetVelocity() {
  ...
  switch (position) {
    ...
    case P1:  // 第2点
      ...
      if ((fabs(p0.world_x - p1.world_x) < IS_ZERO) && useUpflow) {
        linear_vel_x = 0;
        isXarrived = true;
      } else {
        linear_vel_x = mecanumXbot.botVel.vel_x;  // m/s
        if (fabs(botOdometry.botPosition.position_x - p1.world_x) < 0.001) {
          isXarrived = true;
        }
      }
      // 上一点坐标等于目标点目标，并开启光流时，信任光流，速度为零
      if ((fabs(p0.world_y - p1.world_y) < IS_ZERO) && useUpflow) {
        linear_vel_y = 0;
        isYarrived = true;
      } else {
        linear_vel_y = mecanumXbot.botVel.vel_y;  // m/s
        if (fabs(botOdometry.botPosition.position_y - p1.world_y) < 0.001) {
          isYarrived = true;
        }
      }
      ...
      // angular_vel_z = 0;  // rad/s
      angular_vel_z = mecanumXbot.botVel.angular_vel_z;
      if (fabs(botOdometry.botPosition.heading_theta - p1.world_angular_z) < 0.01) {
        isZarrived = true;
      }
      if ((currentMillis - previousMillis >= 60000) || (isXarrived && isYarrived && isZarrived)) {
        motors.motorsBrake();
        //关闭光流
        useUpflow = false;
        previousMillis = currentMillis;
    ...
    case P2:  // 第3点
      ...
      useUpflow = true;
      // 上一点坐标等于目标点目标，并开启光流时，信任光流，速度为零
      if ((fabs(p1.world_x - p2.world_x) < IS_ZERO) && useUpflow) {
        linear_vel_x = 0;
      } else {
        linear_vel_x = mecanumXbot.botVel.vel_x;  // m/s
      }
      // 上一点坐标等于目标点目标，并开启光流时，信任光流，速度为零
      if ((fabs(p1.world_y - p2.world_y) < IS_ZERO) && useUpflow) {
        linear_vel_y = 0;
      } else {
        linear_vel_y = mecanumXbot.botVel.vel_y;  // m/s
      }
