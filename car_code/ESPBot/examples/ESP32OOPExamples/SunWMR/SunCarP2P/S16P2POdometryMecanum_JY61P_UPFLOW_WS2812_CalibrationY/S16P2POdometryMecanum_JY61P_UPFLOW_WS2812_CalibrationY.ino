...
void setup() {
  ...
  delay(100);
  //更换地面条件，要重新校准
  botOdometry.calibrationK(1.0, 0.9762, 1.0);
#ifdef USE_OLED
  u8g2.begin();
...
void updateTargetVelocity() {
  ...
  switch (position) {
    ...
    case P1:  // 第2点
    //清里程计X值
        if ((abs(p0.world_x - p1.world_x) < IS_ZERO) ) {
botOdometry.botPosition.position_x=p1.world_x;  }
        if ((abs(p0.world_y - p1.world_y) < IS_ZERO) ) {
botOdometry.botPosition.position_y=p1.world_y;  }
      ...
      if ((abs(p0.world_x - p1.world_x) < IS_ZERO) && useUpflow) {
        ...
        linear_vel_x = mecanumXbot.botVel.vel_x;  // m/s
        if (abs(botOdometry.botPosition.position_x - p1.world_x) < 0.001) {
          isXarrived = true;
        }
      }
      // 上一点坐标等于目标点目标，并开启光流时，信任光流，速度为零
      if ((abs(p0.world_y - p1.world_y) < IS_ZERO) && useUpflow) {
        ...
        linear_vel_y = mecanumXbot.botVel.vel_y;  // m/s
        if (abs(botOdometry.botPosition.position_y - p1.world_y) < 0.001) {
          isYarrived = true;
        }
      }
      ...
      // angular_vel_z = 0;  // rad/s
      angular_vel_z = mecanumXbot.botVel.angular_vel_z;
      if (abs(botOdometry.botPosition.heading_theta - p1.world_angular_z) < 0.01) {
        isZarrived = true;
      }
