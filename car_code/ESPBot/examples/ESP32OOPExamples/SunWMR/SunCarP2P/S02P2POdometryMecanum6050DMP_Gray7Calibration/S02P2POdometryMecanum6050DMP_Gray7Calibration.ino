...
Grayscale GraySensors; //使用默认构造函数。
// uart输出量的结构体
...
float last_position_x, current_position_x;
...
CAR_GOAL_POINT p1{1230, 900, 0};
CAR_GOAL_POINT p2{600, 900, 0};
//比例系数和速度最大值
CAR_KPS_MAX botKps{0.01, 0.01, 5, 0.2, 0.2, 3};
//运行状态标记
enum CARMOTION
{
  P0,
...
const long period = 8000;         // period at which to run in ms
/***************** 定时中断参数 *****************/
Ticker timer1; // 中断函数
bool timer_flag = 0;
//******************创建4个编码器实例***************************//
...
// OLED初始化
  ...
  Serial.println("Sunnybot 麦轮P2P测试，请按下对应按键开始测试");
#ifdef USE_OLED
  u8g2.clearBuffer();                         // clear the internal memory
...
void loop()
{
  ...
  if (GrayUartOutputIO.ioCount > 5)
  {
    ...
    if (isGrayFirst)
    {
      isGrayFirst = 0;
      blacklineCount++;
      current_position_x = botOdometry.botPosition.position_x;
    }
    ...
    else
    {
      ...
      if (currenttIOSTATUS.getTime - lastIOSTATUS.getTime > 1000)
      {
        ...
        current_position_x = botOdometry.botPosition.position_x;
        float x_diff = current_position_x - last_position_x;
        if (x_diff > 250.0)
        {
          botOdometry.botPosition.position_x = last_position_x + 300.0;
        }
        else if (x_diff < -250.0)
        {
          botOdometry.botPosition.position_x = last_position_x - 300.0;
        }
        else if (fabs(x_diff) < 50.0)
        {
          // botOdometry.botPosition.position_x=last_position_x;
        }
      }
    }
  }
