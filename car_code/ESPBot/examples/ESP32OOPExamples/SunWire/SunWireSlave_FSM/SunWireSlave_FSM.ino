...
// 比例系数和速度最大值
CAR_KPS_MAX botKps{0.01, 0.01, 1, 0.3, 0.3, 3};
// 目标到达标志
...
void timerISR()
{
  ...
  motors.setSpeeds(outPWM[0], outPWM[1], outPWM[2], outPWM[3]); // 刹车状态控制
}
...
void updateTargetVelocity(int8_t ponit_n)
{
  ...
  if (max_time_flag)
  {
    ...
    x_max_time = int(abs(cpx - point[ponit_n].world_x) / botKps.MAX_x + 2000);
    y_max_time = int(abs(cpy - point[ponit_n].world_y) / botKps.MAX_y + 2000);
    z_max_time = int(abs(botOdometry.botPosition.heading_theta - point[ponit_n].world_angular_z) / botKps.MAX_z  + 2000);
  }
  ...
  mecanumXbot.getBotVel(point[ponit_n], botKps);
    ...
    angular_vel_z = mecanumXbot.botVel.angular_vel_z;
        if ((currentMillis - previousMillis >= x_max_time) )
    {
      isYarrived = true;
      isXarrived = true;
      isZarrived = true;
  ...
  case Y:
    ...
    linear_vel_y = mecanumXbot.botVel.vel_y; // m/s
    angular_vel_z = 0;
    if ((currentMillis - previousMillis >= y_max_time) || fabs(botOdometry.botPosition.position_y - point[ponit_n].world_y) < IS_ZERO)
    {
      isYarrived = true;
      motion_direction = X;
      previousMillis = currentMillis;
  ...
  case X:
    ...
    linear_vel_y = 0;                        // m/s
    angular_vel_z = 0;
    if ((currentMillis - previousMillis >= x_max_time) || fabs(botOdometry.botPosition.position_x - point[ponit_n].world_x) < IS_ZERO)
    {
      isXarrived = true;
      motion_direction = Z;
      previousMillis = currentMillis;
  ...
  case Z:
    ...
    linear_vel_y = 0; // m/s
    angular_vel_z = mecanumXbot.botVel.angular_vel_z;
    if ((currentMillis - previousMillis >= z_max_time) || fabs(botOdometry.botPosition.heading_theta - point[ponit_n].world_angular_z) < IS_ZERO)
    {
      isZarrived = true;
      motion_direction = Y;
      previousMillis = currentMillis;
