/*
Name:    SFM_BDCMotorEncoder.ino 有刷直流电机开环4路电机测速
原理：使用编码器测速
Created: 20210914 8:39:28 PM
Author: Sunny
*/
...
const int CIN1 = 12;//C电机控制PWM波
const int CIN2 = 11;//C电机控制PWM波
...
void control() {
  ...
  Velocity_B = -Velocity_2;    Velocity_2 = 0; //读取编码器数据并根据实际接线做+-调整、然后清零，这就是通过M法测速（单位时间内的脉冲数）得到速度。
  Velocity_C = Velocity_3;    Velocity_3 = 0; //读取编码器数据并根据实际接线做+-调整、然后清零，这就是通过M法测速（单位时间内的脉冲数）得到速度。
...
void setup() {
  ...
  pinMode(DIRECTION_B, INPUT_PULLUP);
  pinMode(DIRECTION_C, INPUT_PULLUP);
