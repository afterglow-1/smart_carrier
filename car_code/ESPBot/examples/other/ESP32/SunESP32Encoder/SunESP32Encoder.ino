...
void readMotor1EncoderA() {
  if (digitalRead(Motor1EncoderA) == LOW) {     //如果是下降沿触发的中断
    if (digitalRead(Motor1EncoderB) == LOW)      M1EncoderCount--;  //根据另外一相电平判定方向
    else      M1EncoderCount++;
  }
  else {     //如果是上升沿触发的中断
    if (digitalRead(Motor1EncoderB) == LOW)      M1EncoderCount++; //根据另外一相电平判定方向
...
void readMotor1EncoderB() {
  if (digitalRead(Motor1EncoderB) == LOW) {     //如果是下降沿触发的中断
    if (digitalRead(Motor1EncoderA) == LOW)      M1EncoderCount++;  //根据另外一相电平判定方向
    else      M1EncoderCount--;
  }
  else {     //如果是上升沿触发的中断
    if (digitalRead(Motor1EncoderA) == LOW)      M1EncoderCount--; //根据另外一相电平判定方向
...
void readMotor2EncoderA() {
  if (digitalRead(Motor2EncoderA) == LOW) {     //如果是下降沿触发的中断
    if (digitalRead(Motor2EncoderB) == LOW)      M2EncoderCount--;  //根据另外一相电平判定方向
    else      M2EncoderCount++;
  }
  else {     //如果是上升沿触发的中断
    if (digitalRead(Motor2EncoderB) == LOW)      M2EncoderCount++; //根据另外一相电平判定方向
...
void readMotor2EncoderB() {
  if (digitalRead(Motor2EncoderB) == LOW) {     //如果是下降沿触发的中断
    if (digitalRead(Motor2EncoderA) == LOW)      M2EncoderCount++;  //根据另外一相电平判定方向
    else      M2EncoderCount--;
  }
  else {     //如果是上升沿触发的中断
    if (digitalRead(Motor2EncoderA) == LOW)      M2EncoderCount--; //根据另外一相电平判定方向
...
attachInterrupt(Motor1EncoderA, readMotor1EncoderA, CHANGE);//中断处理函数，电平变化触发
attachInterrupt(Motor1EncoderB, readMotor1EncoderB, CHANGE);//中断处理函数，电平变化触发
attachInterrupt(Motor2EncoderA, readMotor2EncoderA, CHANGE);//中断处理函数，电平变化触发
attachInterrupt(Motor2EncoderB, readMotor2EncoderB, CHANGE);//中断处理函数，电平变化触发
...
Serial.print("左轮M1旋转的角度为： ");
 Serial.print(M1Velocity);
 Serial.print("右轮M2旋转的角度为： ");
 Serial.println(M2Velocity);
 delay(100);
