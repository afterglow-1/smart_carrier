...
TTL_Stepper::TTL_Stepper()
    : protocol(nullptr), addr(0), convert_K(1.0f), substep(1), baudrate(0),
      Speed(0), ptime(0), previousPosition(0), currentPosition(0), delta(0),
      Acceleration(1), Func_Code(0), recDate{0}, Date_Index(0),
      Ask_State(false), CW(true), conditionNotMet(false), command_check(false),
      enstate(false), onPos_state(false), locked_state(false),
      loPro_state(false), snF(false)
{
}
...
void TTL_Stepper::runToNewPosition(float x)
{
  ...
  uint32_t clk = fabs(x) * substep * 200.0f / convert_K;
  uint8_t direction = (x >= 0.0f) ? CW : !CW;
  do
  {
    currentPosition = previousPosition + Calculate_DeltaPos(); // 更新当前绝对位置
    previousPosition = currentPosition;                        // 更新运动开始位置
    protocol->Emm_V5_Pos_Control(this->addr, direction, Speed, Acceleration, clk, 1, 0);
    ptime = millis(); // 用于后续更新位移量
  } while (!wrong_command_catch());
  delta = x - previousPosition; // 更新位移量
}
/**
 ...
 * @retval
 */
bool TTL_Stepper::wait(uint32_t timeoutMs)
{
  onPos_state = false;
  Ask_State = false;
  Date_Index = 0;
  ...
  while (!onPos_state)
  {
    // Check before polling so a noisy UART cannot postpone the timeout.
...
/**
 * @brief    计算当前增加的位置量,有问题
 * @param
 * @retval   位移量
 */
float TTL_Stepper::Calculate_DeltaPos()
{
  uint32_t total_pulse = fabs(delta * substep * 200 * 1.0 / convert_K); // 正负号转换
  uint32_t delta_time = millis() - ptime;
  double v_max = Speed * substep * 200.0 / 60000.0; // 脉冲数每毫秒
  double s = 0.0;
  double a = 0.0;
  if (this->Acceleration == 0)
  {
    double total_time = total_pulse / v_max;
    if (delta_time < 0)
    {
      s = 0.0;
    }
    else if (delta_time <= total_time)
    {
      s = v_max * delta_time;
    }
    else
  ...
  else
  {
    ...
    double s_min = 0.5 * (v_max * v_max) / a; // 单段加减速最小位移

    // 梯形加减速（有匀速段）
    if (total_pulse >= 2 * s_min)
    {
      double t_acc = v_max / a;                 // 加速时间
      double s_const = total_pulse - 2 * s_min; // 匀速段位移
      double t_const = s_const / v_max;         // 匀速时间
      double t_total = 2 * t_acc + t_const;     // 总时间

      if (delta_time <= t_acc)
      {
        // 加速段：s = 0.5 * a * t²
        s = 0.5 * a * delta_time * delta_time;
      }
      else if (delta_time <= t_acc + t_const)
      {
        // 匀速段：s = s_acc + v_max*(t-t_acc)
        s = 0.5 * a * t_acc * t_acc + v_max * (delta_time - t_acc);
      }
      else if (delta_time <= t_total)
      {
        // 减速段：s = s_acc + s_const + [v_max*t_dec - 0.5*a*t_dec²]
        double t_dec = delta_time - (t_acc + t_const);
        s = 0.5 * a * t_acc * t_acc + v_max * t_const + (v_max * t_dec - 0.5 * a * t_dec * t_dec);
      }
      else
      {
        s = total_pulse; // 运动结束
    ...
    {
      double v_peak = sqrt(a * total_pulse); // 实际峰值速度
      double t_acc = v_peak / a;             // 加速时间
      double t_total = 2 * t_acc;            // 总时间
      ...
      {
        // 减速段：s = s_acc + [v_peak*t_dec - 0.5*a*t_dec²]
        double t_dec = delta_time - t_acc;
        s = 0.5 * a * t_acc * t_acc + (v_peak * t_dec - 0.5 * a * t_dec * t_dec);
      }
      else
      {
        s = total_pulse; // 运动结束
  ...
  {
    return -s * convert_K * 1.0 / (substep * 200.0);
  }
}
/**
 * @brief    计算当前位置，
...
void TTL_Stepper::init()
{
  ...
  Ask_State = false;
  onPos_state = false;
  recDate[0] = recDate[1] = recDate[2] = recDate[3] = 0;
...
void TTL_Stepper::state_update()
{
  if (Ask_State)
  {
    // Bound work per call so wait() always gets a chance to check timeout.
    uint8_t bytesProcessed = 0;
    while (protocol->serial->available() && bytesProcessed < 32)
    {
      bytesProcessed++;
      if (Date_Index >= sizeof(recDate))
      {
        recDate[0] = recDate[1];
        recDate[1] = recDate[2];
        recDate[2] = recDate[3];
        Date_Index = sizeof(recDate) - 1;
      }
...
/**
 * @brief    错误命令判断，通过步进返回值判断之前是否发出了错误命令
 * @param
 * @retval   0：发出了错误命令；1发出的是正确命令
 */
bool TTL_Stepper::wrong_command_catch()
{
  uint8_t result = protocol->error_address_catch();
  ...
  if (result == 254)
  {
    ...
    // 返回true只是为了退出runToNewPosition()的do-while，
    // 防止程序再次永久阻塞。
    return true;
  }
 ...
 * @retval   地址 + 功能码 + 命令状态 + 校验字节
 */
void TTL_Protocol::Emm_V5_Vel_Control(uint8_t addr, uint8_t dir, uint16_t vel, uint8_t acc, bool snF)
{
  uint8_t cmd[16] = {0};
...
/**
 * @brief    仅确认是否发出错误命令，返回一个错误命令步进地址，否则返回255，添加条件不满足
 * @param
 * @retval   错误命令步进地址
 */
uint8_t TTL_Protocol::error_address_catch()
{
  ...
  {
    i++;
    if (i >= static_cast<int>(sizeof(rxCmd)))
    {
      return 254;
    }
    rxCmd[i] = serial->read();
