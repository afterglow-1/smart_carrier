...
public:
   ...
   float previousPosition; // 运动开始的位置
   float currentPosition;  // 当前绝对位置
   float delta;            // 运动需要的位移,用以更新实时位置
   uint8_t Acceleration;   // 加速度(0~255)
   uint8_t Func_Code;      // 命令码
   uint8_t recDate[4];     // 返回值接收区
   ...
   void update_state(); // 状态更新（使能状态、到位状态、堵转、堵转保护）
   void init();         // 初始化
   ...
   float Calculate_DeltaPos();   // 计算当前位移
   float Calculate_CurrentPos(); // 计算当前绝对位置
   void update_CurrentPos();     // 更新当前位置；
   void runToOrigin();           // 回零
   bool wait(uint32_t timeoutMs = 10000);
   bool enable();                  // 电机使能
