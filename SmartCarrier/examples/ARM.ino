...
bool moveSmallArm(float distance);
bool moveBigArm(float distance);
void closeGripper();
void openGripper();
...
const float RACK_TRAVEL_PER_REV = PI * GEAR_MODULE * GEAR_TEETH; // 齿轮转一圈的齿条位移，mm
const uint16_t SMALL_ARM_SPEED_RPM = 10; // 伸缩轴速度
const uint16_t BIG_ARM_SPEED_RPM = 30;   // 丝杆升降轴速度
const uint8_t ARM_ACCELERATION = 50;     // 0为立即启动，数值越大加速越快
const bool SMALL_ARM_POSITIVE_DIR = 0;   // 伸出方向
const bool BIG_ARM_POSITIVE_DIR = 1;     // 上升方向；若实机向下运动则改为0
...
void setup() {
 delay(100);//等待步进上电启动 
  ...
  small_armStepper.set(SMALL_ARM_SPEED_RPM, ARM_ACCELERATION,
                       SMALL_ARM_POSITIVE_DIR,
                       RACK_TRAVEL_PER_REV * 10.0f, 16);
  big_armStepper.set(BIG_ARM_SPEED_RPM, ARM_ACCELERATION,
                     BIG_ARM_POSITIVE_DIR,
                     LEAD_SCREW_PITCH * 10.0f, 16);
  small_armStepper.enable();
  delay(20);
  Stepper_protocol.emptyCache();
  big_armStepper.enable();
  delay(20);
  Stepper_protocol.emptyCache();
...
void loop() {
  ...
  // 悬臂轴伸出 10mm
  Serial.println("Small arm: moving to 10 mm");
  if (!moveSmallArm(10)) {
    Serial.println("Small arm feedback timeout; continuing test.");
  }
  Serial.println("Small arm: done");
  delay(2000);

  // 丝杠升降轴上升 10mm
  Serial.println("Big arm: moving up to 10 mm");
  if (!moveBigArm(10)) {
    Serial.println("Big arm feedback timeout; continuing test.");
  }
  Serial.println("Big arm: done");
  delay(2000);
  ...
  // 回到初始位置
  rotateBase(0);
  if (!moveSmallArm(0) || !moveBigArm(0)) {
    Serial.println("Arm return feedback timeout.");
  }
  delay(2000);
...
// 悬臂轴移动函数
bool moveSmallArm(float distance) {
  small_armStepper.runToNewPosition(distance * 10.0f);
  return small_armStepper.wait();
}

// 丝杠升降轴移动函数
bool moveBigArm(float distance) {
  big_armStepper.runToNewPosition(distance * 10.0f);
  return big_armStepper.wait();
}
