...
void setup() {
  ...
  while (Serial_TJCHMI.read() >= 0)
    ;                                            //清空串口缓冲区
  Serial_TJCHMI.print("page QR\xff\xff\xff");  //发送命令让屏幕跳转到main页面
...
if(scanFlag)
{
  Serial_TJCHMI.print("QR.t1.txt=\"QROK\"\xff\xff\xff");
      //刷新屏幕显示
    char strTemp[20];
    sprintf(strTemp, "QR.t2.txt=\"%s\"\xff\xff\xff", receivedData);
    //把字符串发送出去
    ...
    if(!firstFlag){
  Serial_TJCHMI.print("QR.t2.txt=\"000+000+000+000\"\xff\xff\xff");
  firstFlag=true;
  }
}
